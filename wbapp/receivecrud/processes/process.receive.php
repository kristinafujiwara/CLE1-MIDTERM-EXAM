...
function create_new_transaction(){
   ...
   $name= ucwords($_POST['sname']);
    ...
    if(is_numeric($rid)){
        header('location: ../index.php?page=payroll&action=receive&id='.$rid);
    }
}
...
function new_receive_item(){
    ...
    if($rid){
        header('location: ../index.php?page=payroll&action=receive&id='.$recid);
    }
}
...
function save_receive_items(){
    ...
    if($rid){
        header('location: ../index.php?page=payroll&action=receive&id='.$id);
    }
}